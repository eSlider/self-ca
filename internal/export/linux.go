...
func LinuxInstallScript(in CAInput) (string, error) {
	...
	fmt.Fprintf(&b, "    sudo cp \"$CA_FILE\" \"/usr/local/share/ca-certificates/%s.crt\"\n", safeName)
	b.WriteString("    sudo update-ca-certificates\n")
	b.WriteString("    ;;\n")
	b.WriteString("  fedora|rhel|centos|rocky|almalinux|ol|amzn)\n")
	fmt.Fprintf(&b, "    sudo cp \"$CA_FILE\" \"/etc/pki/ca-trust/source/anchors/%s.pem\"\n", safeName)
	b.WriteString("    sudo update-ca-trust extract\n")
	b.WriteString("    ;;\n")
	b.WriteString("  arch|manjaro|garuda|steamos|endeavouros)\n")
	fmt.Fprintf(&b, "    sudo cp \"$CA_FILE\" \"/etc/ca-certificates/trust-source/anchors/%s.crt\"\n", safeName)
